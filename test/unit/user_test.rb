...
class LoginTest < ActiveSupport::TestCase
  ...
  def test_should_reset_password
    ...
    assert_equal users(:quentin), Login.authenticate('quentin', 'new password')
  end

  def test_should_not_rehash_password
    users(:quentin).update_attributes(:login => 'quentin2')
    assert_equal users(:quentin), Login.authenticate('quentin2', 'monkey')
  end

  def test_should_authenticate_login
    assert_equal users(:quentin), Login.authenticate('quentin', 'monkey')
  end
...
protected
  def create_login(options = {})
    record = Login.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.save
    record
  end
end
