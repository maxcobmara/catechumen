class Exammark < ActiveRecord::Base
  ...
  def self.update_multiple_exam1mark_in_grades(exammaker_id,all_exammarks,selected_subject,students)
     @examtype = Exammaker.find(exammaker_id).examtype
     @counter = 0
     students.each do |student|
        @grades1 = Grade.find(:all, :conditions=> ['student_id=? and subject_id=?', student.id, selected_subject])  #return array
        @grades1.each do |grade|
          @sum_of_marks = Exammark.sum_marks(all_exammarks[@counter.to_s][:marks_attributes])
          grade.exam1marks = @sum_of_marks
          grade.save if grade.exam1marks && @examtype == "2"
          @counter+=1
        end
      end
  end
