class TravelRequest < ActiveRecord::Base
  ...
  #controller searches
  def self.in_need_of_approval(search)
    if search !=nil
      for_approvals = TravelRequest.find(:all, :conditions => ['hod_id = ? AND is_submitted = ? AND (hod_accept IS ? OR hod_accept = ?) AND destination ILIKE(?)', Login.current_login.staff_id, true, nil, false, "%#{search}%"],:order=> 'depart_at asc')
    else
      for_approvals = TravelRequest.find(:all, :conditions => ['hod_id = ? AND is_submitted = ? AND (hod_accept IS ? OR hod_accept = ?)', Login.current_login.staff_id, true, nil, false],:order=> 'depart_at asc')
    end 
    for_approvals
  end
  
  def self.my_travel_requests(search)
    if search!=nil 
      travel_requests = TravelRequest.find(:all, :conditions => ['staff_id = ? AND destination ILIKE(?)', Login.current_login.staff_id, "%#{search}%"], :order=> 'depart_at asc')
    else
      travel_requests = TravelRequest.find(:all, :conditions => ['staff_id = ?', Login.current_login.staff_id], :order=> 'depart_at asc')
    end 
    travel_requests
  end
...
#      end
     #override all above approver - 23Dec2014 - do not remove above yet, may be useful for other submodules #refer UAT dated 4th January 2013, item 1e.
