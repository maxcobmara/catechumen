class Appraisal < ActiveRecord::Base
  ...
  belongs_to :ppk, :class_name => 'Staff', :foreign_key => 'ppk_id'
  ...
  accepts_nested_attributes_for :strainings, :reject_if => lambda { |a| a[:place].blank? }
  
  has_many :trainneeds, :dependent => :destroy
  accepts_nested_attributes_for :trainneeds, :reject_if => lambda { |a| a[:reason].blank? }
 ...
 #----------------------------------------Code for staff that no longer exist in database---------------- 
  ...
  def staffname_details 
       ...
       if staff_id == nil
         ...
         appraisedstaff.staff_name_with_title
       end
  end
