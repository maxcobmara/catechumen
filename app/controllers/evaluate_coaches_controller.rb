class EvaluateCoachesController < ApplicationController
  ...
  def report_evaluate_instructor
     @evaluate_coaches = EvaluateCoach.find(:all, :order => "id", :limit => 20)
     @evaluate_programmes =  @evaluate_coaches.group_by {|t| t.programme_details}
      render :layout => 'report'
  end
end
