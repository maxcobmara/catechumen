class Exammakeranalysis < ActiveRecord::Base
  validates_uniqueness_of :exammaker_id, :message => "can be analysed only once."
  ...
  def self.set_array(students_list,no,selected_subject)
    ...
    students_list.each do |student| 
      student.studentgrade.each do |student_grade| 
    		if student_grade.subjectgrade.id == selected_subject 
      	  @student_ca_mse << student_grade.ca_plus_mse.to_f if no == 1
  ...
  def self.statistic_item(column,loop_column)
     @statistic_item = [] 
		 ...
		 @statistic_item << Math.sqrt(@sample_variance).to_f 
     ...
     @pass_rate = Exammakeranalysis.set_pass_rate(column,loop_column)
